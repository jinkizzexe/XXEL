...
async function startgss() {
    ...
    const gss = gssConnect({
        ...
        browser: ['Vibranium','Safari','1.0.0'],
        auth: state,
        getMessage: async (key) => {
            ...
            return {
                conversation: " *Hɪ! Iᴍ Vɪʙʀᴀɴɪᴜᴍ, ʜᴏᴡ ᴀʀᴇ ʏᴏᴜ?* "
            }
        }
    })
...
gss.ev.on("call", async (json) => {
    ...
    gss.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./vibranium.js")(gss, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
gss.ev.on('messages.update', async chatUpdate => {
    ...
    if (connection === 'close') {
        ...
        gss.sendMessage('8801928502256@s.whatsapp.net', {
            text: ` *Hɪ Sɪʀ! ɪᴍ ᴜsɪɴɢ Vɪʙʀᴀɴɪᴜᴍ ʀɪɢʜᴛ ɴᴏᴡ!* `
        });
    }
});
...
async function setBio() {
    ...
    const status = `💠 ${timeString} Iᴍ ʙᴜsʏ (ᴅʀᴇᴀᴍɪɴɢ ᴏғ ʏᴏᴜ) <3~\n`;
