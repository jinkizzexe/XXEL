               ...
               let xp = 'exp' in plugin ? parseInt(plugin.exp) : 17 // XP Earning per command
                if (xp > 200)
                    ...
                    if (!isPrems && plugin.credit && global.db.data.users[m.sender].credit < plugin.credit * 1) {
                        this.reply(m.chat, `🟥 Yoᥙ dont hᥲvᥱ ᥱnoᥙgh goᥣd for thιs`, m)
    ...
    switch (action) {
        case 'add':
            if (chat.welcome) {
              ...
              for (let user of participants) {
                ...
                try {
                  ...
                  pp = 'https://i.imgur.com/ycarEW8.jpg'; // Assign default image URL
                  ppgp = 'https://i.imgur.com/ycarEW8.jpg'; // Assign default image URL
                } finally {
                  ...
                  let secondText = `Wᴇʟᴄᴏᴍᴇ , ${await this.getName(user)}, ᴏᴜʀ ${nthMember}ᴛʜ ᴍᴇᴍʙᴇʀ`;
                  ...
                  try {
                    ...
                    this.sendMessage(id, {
                        ...
                        externalAdReply: {
                        title: "VɪʙʀAɴɪUM -ʙOT",
                        body: "Wᥱᥣᥴomᥱ to Groᥙρ",
                        thumbnailUrl: welcomeApiUrl,
                        sourceUrl: 'https://chat.whatsapp.com/HGnDq9xzEfDCl5Irj80QA3',
                        mediaType: 1,
          ...
          case 'remove':
            if (chat.welcome) {
              ...
              for (let user of participants) {
                ...
                try {
                  ...
                  let nthMember = groupMetadata.participants.length;
                  let secondText = `Gᴏᴏᴅʙʏᴇ, ᴏᴜʀ ${nthMember}ᴛʜ ɢʀᴏᴜᴘ ᴍᴇᴍʙᴇʀ`;
            ...
            break;
            case "promote":
                const promoteText = (chat.sPromote || this.spromote || conn.spromote || `${emoji.promote} @user *ιs noᥕ ᥲn ᥲdmιn*`).replace("@user", "@" + participants[0].split("@")[0]);
                if (chat.detect) {
                    this.sendMessage(id, {
                ...
                break;
            case "demote":
                const demoteText = (chat.sDemote || this.sdemote || conn.sdemote || `${emoji.demote} @user *ιs no ᥣongᥱr ᥲn ᥲdmιn*`).replace("@user", "@" + participants[0].split("@")[0]);
                if (chat.detect) {
                    this.sendMessage(id, {
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.desc) {
            text = (chats.sDesc || this.sDesc || conn.sDesc || `*${emoji.desc} Dᥱsᥴrιρtιon hᥲs bᥱᥱn ᥴhᥲngᥱd to*\n@desc`)
                .replace("@desc", groupUpdate.desc)
        } else if (groupUpdate.subject) {
            text = (chats.sSubject || this.sSubject || conn.sSubject || `*${emoji.subject} Sᥙbjᥱᥴt hᥲs bᥱᥱn ᥴhᥲngᥱd to*\n@subject`)
                .replace("@subject", groupUpdate.subject)
        } else if (groupUpdate.icon) {
            text = (chats.sIcon || this.sIcon || conn.sIcon || `*${emoji.icon} Iᥴon hᥲs bᥱᥱn ᥴhᥲngᥱd*`)
                .replace("@icon", groupUpdate.icon)
        } else if (groupUpdate.revoke) {
            text = (chats.sRevoke || this.sRevoke || conn.sRevoke || `*${emoji.revoke} Groᥙρ ᥣιnk hᥲs bᥱᥱn ᥴhᥲngᥱd to*\n@revoke`)
                .replace("@revoke", groupUpdate.revoke)
        } else if (groupUpdate.announce === true) {
            text = (chats.sAnnounceOn || this.sAnnounceOn || conn.sAnnounceOn || `*${emoji.announceOn} Groᥙρ ιs noᥕ ᥴᥣosᥱd!*`)
        } else if (groupUpdate.announce === false) {
            text = (chats.sAnnounceOff || this.sAnnounceOff || conn.sAnnounceOff || `*${emoji.announceOff} Groᥙρ ιs noᥕ oρᥱn!*`)
        } else if (groupUpdate.restrict === true) {
            text = (chats.sRestrictOn || this.sRestrictOn || conn.sRestrictOn || `*${emoji.restrictOn} Groᥙρ ιs noᥕ rᥱstrιᥴtᥱd to ρᥲrtιᥴιρᥲnts onᥣყ!*`)
        } else if (groupUpdate.restrict === false) {
            text = (chats.sRestrictOff || this.sRestrictOff || conn.sRestrictOff || `*${emoji.restrictOff} Groᥙρ ιs noᥕ rᥱstrιᥴtᥱd to ᥲdmιns onᥣყ!*`)
        }
...
global.dfail = (type, m, conn) => {
    const userTag = `🐦 Hᥱᥣᥣo *@${m.sender.split("@")[0]}*, `
    const emoji = {
        general: '⚙️',
        owner: '👑',
        moderator: '🛡️',
    ...
    const msg = {
        owner: `*${emoji.owner} Oᥕnᥱr's ρoᥕᥱr*\n
    ${userTag} Thιs ᥴommᥲnd onᥣყ for mყ bᥱᥣovᥱd *oᥕnᥱr*!`,
        moderator: `*${emoji.moderator} Modᥱrᥲtor's ρoᥕᥱr*\n
    ${userTag} Thιs ᥴommᥲnd ᥴᥲn onᥣყ  bᥱ ᥙsᥱd bყ *Modᥱrᥲtors*!`,
        premium: `*${emoji.premium} Prᥱmιᥙm ρoᥕᥱr*\n
    ${userTag} Yoᥙ nᥱᥱd *Prᥱmιᥙm Pᥲss* to ᥙsᥱ thιs ᥴommᥲnd!`,
        group: `*${emoji.group} Groᥙρs ρoᥕᥱr*\n
    ${userTag} Thιs ᥴommᥲnd ᥴᥲn onᥣყ bᥱ ᥙsᥱd ιn *Groᥙρ-Chᥲts*!`,
        private: `*${emoji.private} Prιvᥲtᥱ ρoᥕᥱr*\n
    ${userTag} Thιs ᥴommᥲnd ᥴᥲn onᥣყ bᥱ ᥙsᥱd ιn *Prιvᥲtᥱ Chᥲts*!`,
        admin: `*${emoji.admin} Admιn's ρoᥕᥱr*\n
    ${userTag} Onᥣყ groᥙρ *ᥲdmιn* ᥴᥲn ᥙsᥱ thιs ᥴommᥲnd!`,
        botAdmin: `*${emoji.botAdmin} Bot ᥲdmιn's ρoᥕᥱr*\n
    ${userTag} Mᥲkᥱ thᥱ bot ᥲn *Admιn* to ᥙsᥱ thιs ᥴommᥲnd!`,
        unreg: `*${emoji.unreg} Rᥱgιstrᥲtιon rᥱqᥙιrᥱd*\n
    ${userTag} Pᥣᥱᥲsᥱ rᥱgιstᥱr to ᥙsᥱ thιs fᥱᥲtᥙrᥱ bყ tყριng:\n\n *${usedPrefix}ʀᴇɢɪsᴛᴇʀ ɴᴀᴍᴇ.ᴀɢᴇ*\n\nExᥲmρᥣᥱ: *${usedPrefix}register ${m.name}.19*!`,
        nsfw: `*${emoji.nsfw} NSFW ρoᥕᥱr*\n
    ${userTag} NSFW ιs not ᥲᥴtιvᥱ...ᥲsk ᥲdmιns to tᥙrn on nsfᥕ bყ tყριng:\n\n *${usedPrefix}ᴏɴ ɴsғᴡ*`,
        restrict: `*${emoji.restrict} ιnᥲᥴtιvᥱ fᥱᥲtᥙrᥱ ᥲᥣᥱrt*!\n
    ${userTag} Thιs fᥱᥲtᥙrᥱ ιs noᥕ ᥴᥙrrᥱntᥣყ *dιsᥲbᥣᥱd*!`,
    }
     [type]
